...
#define MOVE_VELOCITY           100    /*counts per cycle (reduced for safety)*/
#define OP_MODE_CSP             8   /*CSP mode*/
#define FINAL_POSITION          1000000 /*Target position to stop at*/
...
void cyclic_task(struct timespec *wakeup_time)
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        if ((status & 0x006F) == 0x0027) {
             if (!is_operational[i]) {
                 current_pos = EC_READ_S32(domain1_pd + offsets[i].current_position);
                 target_pos[i] = current_pos;
                 is_operational[i] = 1;
                 printf("Slave %d initialized position to %d\n", i+1, current_pos);
             }
        } else {
            is_operational[i] = 0;
            all_slaves_ready = 0;
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        {
            /* Only move if ALL slaves are ready */
            if (all_slaves_ready) 
            {
                if (target_pos[i] < FINAL_POSITION)
                {
                    target_pos[i] += MOVE_VELOCITY;
                    if (target_pos[i] > FINAL_POSITION)
                        target_pos[i] = FINAL_POSITION;
                }
                else if (target_pos[i] > FINAL_POSITION)
                {
                    target_pos[i] -= MOVE_VELOCITY;
                    if (target_pos[i] < FINAL_POSITION)
                        target_pos[i] = FINAL_POSITION;
            ...
            /* Else: Hold current position (target_pos remains init value) */
